...
customerRouter.get(
  ...
  async (request: RequestWithUser, response) => {
    try {
      const activeStats =
        await CustomerRepository.countUsersBasedOnTheirStatus("Paid");
      const inactiveStats =
        await CustomerRepository.countUsersBasedOnTheirStatus("Expired");
      const totalSum = await CustomerRepository.getTotalAmountPaid();
      const monthlyTotalSum =
        await CustomerRepository.retrievePaidCustomerAmountByMonth();
...
customerRouter.patch(
  ...
  async (request: RequestWithUser, response) => {
    try {
      const deletedCustomer = await CustomerRepository.softDelete(
        request.params.id!,
        true,
      );
      if (!deletedCustomer) {
      ...
      await LogsRepository.logAction(
        admin!._id.toString(),
        `${admin!.first_name} ${admin?.last_name} removed ${deletedCustomer!.first_name} ${deletedCustomer!.first_name}'s from the gym members list at ${new Date().toISOString()}`,
      );
...
customerRouter.put(
  ...
  async (request: RequestWithUser, response) => {
    try {
      ...
      await LogsRepository.logAction(
        admin!._id.toString(),
        `${admin!.first_name} ${admin?.last_name} updated ${updatedCustomer!.first_name} ${updatedCustomer!.first_name}'s customer details at ${new Date().toISOString()}`,
      );
